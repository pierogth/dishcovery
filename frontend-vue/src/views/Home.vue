...
export default class Home extends Vue {
  ...
  topRated: IArticleList = { articles: [], articlesCount: 0 };
  ...
  async fetchFeed(): Promise<void> {
    ...
    try {
      ...
      this.topRated = await Article.getTopRated();
      /*ottengo la lista degli slug topRated*/
      const slugs = this.topRated.articles.map((article)=>{
        return article.slug;
      });
      ...
      switch (this.activeTabId) {
        case FeedType.Global:
          ...
          /*rimuovo i top rated dalla lista*/
          this.activeFeed.articles = this.activeFeed.articles.filter(x=>!slugs.includes(x.slug))
          break;
        case FeedType.Tag:
